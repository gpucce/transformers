    ...
    def __init__(
        ...
        if len(args.fsdp) > 0:
            ...
            if (
                ...
                and "backward_pos" in self.args.fsdp_config.get("backward_prefetch", [])
            ):
        ...
        if self.place_model_on_device and not getattr(
        ...
        if not training:
            ...
            try:
                ...
                with ContextManagers(
                    [
                        self.autocast_smart_context_manager(cache_enabled=False),
                        torch.no_grad(),
                    ]
                ):
                    if version.parse(
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            if not args.ignore_data_skip:
                ...
                if (
                    ...
                    and not args.disable_tqdm
                    and skip_first_batches is None
                ):
        ...
        if os.path.exists(best_model_path) or os.path.exists(best_safe_model_path):
            if self.deepspeed:
                ...
                if is_sagemaker_mp_enabled():
                    ...
                    if hasattr(model, "base_model") and getattr(
                        model.base_model, "is_8bit_serializable", False
                    ):
                        # If train base_8_bit_models using PEFT & LoRA, assume that adapter have been saved properly.
                        if hasattr(model, "active_adapter") and hasattr(
                            model, "load_adapter"
                        ):
                            if os.path.exists(
                                os.path.join(
    ...
    def init_git_repo(self, at_init: bool = False):
        ...
        try:
            self.repo = Repository(
                self.args.output_dir, clone_from=repo_name, token=self.args.hub_token
            )
